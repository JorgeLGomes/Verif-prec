...
def _legenda_modelos(ax, modelos):
    handles = [Line2D([0], [0], color="black", linestyle=_estilo(m)[0],
                      marker=_estilo(m)[1], label=f"{m} ({'continua' if _estilo(m)[0]=='-' else 'tracejada'})")
               for m in modelos]
...
def plota_continuas(df, janela, saida):
    ...
    cor = {m: c for m, c in zip(modelos, plt.cm.tab10(np.linspace(0, 1, 10)))}
    ...
    _legenda_modelos(axs.ravel()[0], modelos)
...
def plota_categoricas(df, janela, saida):
    ...
    cor = {t: c for t, c in zip(limiares,
                                plt.cm.viridis(np.linspace(0, 0.9, len(limiares))))}
    ...
    # legenda dupla: cor=limiar, estilo=modelo
    h_lim = [Line2D([0], [0], color=cor[t], lw=2, label=f"{t:g} mm")
             for t in limiares]
    h_mod = [Line2D([0], [0], color="black", linestyle=_estilo(m)[0],
                    marker=_estilo(m)[1], label=m) for m in modelos]
    axs.ravel()[0].legend(handles=h_lim + h_mod, loc="best", fontsize=7,
